#!/usr/bin/env bash

### Network tester ###

network_test() {
    [[ "$NETWORK_TEST" == "false" && "$ping_cmd" != "1" ]] && return 0
    st "$M_NET_TEST"
    ping -c 2 -i 0.5 aur.archlinux.org &>/dev/null &
    local pid=$!
    local spin='-\|/'
    while kill -0 $pid 2>/dev/null; do
        local i=$(( (i+1)%4 ))
        printf "\r${CYAN} [%c] $M_WAITING...\033[K${NC}" "${spin:$i:1}"
        sleep 0.1
    done
    printf "\r\033[K"
    if wait $pid; then
        sc "$M_NET_OK"
        log_type="net" && mklog "Network test" "available"
    else
        echo -e "${RED}$ERROR \033[K${NC}$M_NET_ERR"
        log_type="net" && mklog "Network test" "unavailable"
        exit 1
    fi
}

### Basic pacman functions ###

proc_func() {
    tag="$action"
    case "$func" in
        i)
            network_test
            cmd="-S"
            flat_cmd="install"
            tag="install"
            tag_cmd="install"
            log_type="1" && mklog $action $tag_cmd
        ;;
        r)
            cmd=$([[ "$agrmode" == "1" ]] && echo "-Rsn" || echo "-Rs")
            tag="remove"
            [[ "$agrmode" == "1" ]] && echo -e "${RED} $NOTE $M_AGRESSIVE_MODE${NC}"
            flat_cmd="uninstall"
            tag_cmd="remove"
            log_type="1" && mklog $action $tag_cmd
        ;;
        u)            
            network_test
            [[ "$AUTO_SNAPSHOT" == "true" ]] && mksnap  
            [[ "$do_snap" == "1" ]] && mksnap
            cmd="-Syu"
            flat_cmd="update"
            tag="update"
            tag_cmd="update"
            log_type="1" && mklog $action $tag_cmd
        ;;
        s)
            network_test
            cmd="-Ss"
            flat_cmd="search"
            tag="search"
            tag_cmd="search"
            log_type="1" && mklog $action $tag_cmd
        ;;
        q)
            cmd="-Qs"
            [[ "$lsaur" == 1 ]] && cmd="-Qqm"
            flat_cmd="list"
            tag="list"
            tag_cmd="query"
            log_type="1" && mklog $action $tag_cmd
            [[ "$ptbin" == 1 ]] && get_package_path "${final_args[@]}"
        ;;
        *)
            err "var func not defined"
        ;;
    esac
}

### CLY info functions ###

display_version() { echo "cly $ver - release: $rc"; }

inform() {
    local version_padded
#   printf -v version_padded "v%-14s" "$ver"
echo -e "${CYAN}${BOLD}       _..._                         "
echo -e "${CYAN}${BOLD}    .-'_..._''. .---.                "
echo -e "${CYAN}${BOLD}  .' .'      '.\|   |                "
echo -e "${CYAN}${BOLD} / .'           |   |.-.          .- "
echo -e "${CYAN}${BOLD}. '             |   | \ \        / / "
echo -e "${CYAN}${BOLD}| |             |   |  \ \      / /  "
echo -e "${CYAN}${BOLD}| |             |   |   \ \    / /   "
echo -e "${CYAN}${BOLD}. '             |   |    \ \  / /    "
echo -e "${CYAN}${BOLD} \ '.          .|   |     \ '' /     "
echo -e "${CYAN}${BOLD}  '. '._____.-'/|   |      \  /      "
echo -e "${CYAN}${BOLD}    '-.______./ '---'      / /       "
echo -e "${CYAN}${BOLD}                       |'-' /        "
echo -e "${CYAN}${BOLD}                        '..'         "
    echo -e "\n ${YELLOW}$M_VERSION ${CYAN}$version\n ${YELLOW}$M_RELEASE ${CYAN}$rc\n ${YELLOW}$M_MADE_BY ${CYAN}xml.dev $M_AND FreaK\n\n${GREEN} $M_THANKS\n ${NC}"
    exit 0
}

### --help/-h Display function ###

help_message() {
    echo "$H_USAGE: cly [$USG1] [$USG2] [$USG3]"
    echo "$H_USAGE: cly [ -i | -r | -s | -u | -q ] [ -f | --flatpak ] [ --noconfirm ]"
    echo "$H_USAGE: cly [ -q | query ] [ --list-aur ] [ --path-to-binary ]"
    echo "$H_USAGE: cly [ --restore-backup ] [ --path /path/to/backup/file.txt ]"
    echo "$H_USAGE: cly [ --backend ] [ yay | paru ]"
    echo "$H_USAGE: cly [ slog | -sl ] [ --lines X ]"
    echo -e "\n$H_OPTIONS:\n"
    echo "-i, install                       $H_INSTALL"
    echo "-r, remove                        $H_REMOVE"
    echo "-q, query                         $H_QUERY"
    echo "-s, search                        $H_SEARCH"
    echo "-u, update                        $H_UPDATE"
    echo "-o, orphan                        $H_ORPHAN"
    echo "-c, cache                         $H_CACHE"
    echo "-m, mirrors                       $H_MIRRORS"
    echo "-h, --help                        $H_HELP"
    echo "-v, --version                     $H_VERSION"
    echo "-dp, why                          $H_WHY"
    echo "-doc, doctor                      $H_DOCTOR"
    echo "mksnap, --create-snapshot         $H_SNAP"
    echo "-cu, check-upds, --check-updates  $H_CHECK_UPD"
    echo "pin, --ignore                     $H_PIN"
    echo "--info                            $H_INFO"
    echo "--fix-keys                        $H_FIX_KEYS"
    echo "--pacdiff                         $H_PACDIFF"
    echo "-cb, --create-backup              $H_CREATE_BKp"
    echo "-rb, --restore-backup             $H_RESTORE_BKp"
    echo "-ptb, --path-to-binary            $H_PATH_BIN"
    echo "-p, --ping                        $H_PING"
    echo "-vi, --view                       $H_VIEW"
    echo "--no-log                          $H_NO_LOG"
    echo "-dry, --dry-run                   $H_DRY"
    echo "--statistics, stats               $H_STATS"
    echo "-fo, --flatpak-only               $H_FLAT_ONLY"
    echo "--edit-config                     $H_EDITOR"
    echo "-b, --backend                     $H_BACKEND"
    echo ""
    display_version
    exit 0
}
